    ...
    with open(csv_file,'r') as f:
        for line in f:
            ...
            variant_data.append({
                ...
                'f_missing':float(f_missing) if f_missing!='.' else 0.0
            })
...
def create_coverage_heatmap(sample_df,output_file,site_data=None):
    ...
    if duplicates.max()>1:
        ...
        duplicate_pairs=duplicates[duplicates>1].index.tolist()[:5]
    ...
    if site_data:
        ...
        # Create figure with space for site annotations - make annotation bar much narrower
        fig,(ax_annot,ax_heatmap)=plt.subplots(1,2,figsize=(18,10),gridspec_kw={'width_ratios':[0.02,0.98]})
        ...
        ax_annot.set_title('Site',fontsize=8)
        
        # Add site labels on the right side of annotation bar - adjust position to avoid truncation
        for site in unique_sites:
            site_indices=[j for j,s in enumerate(site_annotations) if s==site]
            if site_indices:
                mid_point=(min(site_indices)+max(site_indices))/2
                ax_annot.text(-0.3,mid_point,site,ha='right',va='center', 
