...
# Data quality check
if(any(is.na(gene_based$LOG10P))) {
  warning(sprintf("Warning: %d gene-based tests have NA LOG10P values", sum(is.na(gene_based$LOG10P))))
}
if(any(is.infinite(gene_based$LOG10P))) {
  warning(sprintf("Warning: %d gene-based tests have infinite LOG10P values (p=0)", sum(is.infinite(gene_based$LOG10P))))
}
# BETA should be log-OR (typically between about -5 and 5). If not, column alignment may be wrong.
if("BETA" %in% names(gene_based) && any(abs(gene_based$BETA) > 20, na.rm = TRUE)) {
  warning("Some gene-based BETA values are very large (|BETA| > 20). Check that Regenie output column order matches expected (CHROM GENPOS ... BETA SE LOG10P).")
}
...
# Verify p-values are valid for lambda calculation
pval_check <- all(single_variant$PVAL > 0 & single_variant$PVAL <= 1, na.rm = TRUE)
cat("P-value validation: All p-values in valid range (0,1] =", pval_check, "\n")
...
cat("Interpretation: λ < 1.0 indicates deflation (conservative test statistics)\n")
cat("Possible causes: REGENIE over-correction, QC filtering, or case-control imbalance\n\n")

qq_plot <- ggplot(qq_data, aes(x = expected, y = observed)) +
    geom_point(alpha = 0.6, color = "#2E86AB") +
    geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
    # Add deflation demonstration line (lambda = 0.8 would be flatter)
    geom_abline(intercept = 0, slope = lambda_gc, color = "blue", linetype = "dotted", alpha = 0.7) +
    labs(title = "Q-Q Plot: Single-Variant P-values",
         subtitle = paste("Lambda GC:", round(lambda_gc, 3), "| Deflation (λ < 1.0) indicates test statistics are less significant than expected"),
...
### Suggestive Gene-Based Associations (Raw p < 0.001, FDR ≥ 0.05)
...
# Ensure BETA is numeric for x-axis (log-OR; typical range about -5 to 5)
volcano_plot <- ggplot(gene_based_plot, aes(x = as.numeric(BETA), y = LOG10P, color = Significant)) +
